...
async def get_refresh_token_page(token: str = Depends(get_token_page)):
    ...
    if not refresh_user or datetime.utcnow().timestamp() > refresh_user.expires_at.timestamp():
...
async def refresh_token_page(response: Response, refresh: str = Depends(get_refresh_token_page)):
